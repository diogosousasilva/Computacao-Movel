...
interface AIAssistant {
    ...
    /**
     * The temperature value controls how deterministic or creative the AI model's responses will be.
     * Read from config.properties "TEMPERATURE" key. Defaults to 0.7 if not defined.
    ...
    /**
     * The maximum number of tokens the AI model should generate in its response.
     * Read from config.properties "MAX_TOKENS" key. Defaults to 800 if not defined.
     */
    val maxTokens: Int
    ...
    /**
     * Processes user input for sentiment analysis.
     * Instructs the LLM to evaluate sentiment on a 7-point scale and return
     * a JSON response with "rating" (1-7) and "justification".
     *
     * Scale:
    ...
    suspend fun processSentiment(input: String): String {
        val sentimentPrompt = """
            ...
            Analyze the sentiment of the following text and evaluate it on a 7-point scale:
            1. Very Negative
            2. Negative
            3. Slightly Negative
            4. Neutral
